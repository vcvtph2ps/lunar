...
static bool elf_can_load(const void* buffer, size_t size) {
    if(!buffer || size < 4) {
        LOG_KTRC("elf_can_load: buffer is null or too small, size=%zu\n", size);
        return false;
    }
...
static bool internal_allocate_for_image(vm_address_space_t* address_space, const elf64_elf_header_t* elf_header, const elf64_program_header_t* phdr_cache, elf_image_allocation_t* out_allocation) {
    ...
    LOG_KTRC("target_allocation = 0x%lx, image_size = 0x%lx\n", target_allocation, image_size);
    ...
    if(elf_header->type == ELF_TYPE_DYN) {
        ...
        LOG_KTRC("target_allocation = 0x%lx\n", target_allocation);
    ...
    LOG_KTRC("image_slide = 0x%lx, image_size = 0x%lx\n", allocation.image_offset, image_size);
...
static bool internal_elf_load_image(vm_address_space_t* address_space, elf64_elf_header_t* elf_header, const vfs_path_t* path, elf_loader_info_t* out_loader_info) {
    ...
    for(size_t i = 0; i < elf_header->program_header_count; i++) {
        ...
        LOG_KTRC("phdr[%zu].type = 0x%x\n", i, phdr_cache[i].type);
        LOG_KTRC("phdr[%zu].vaddr = 0x%lx, mem_size = 0x%lx, file_size = 0x%lx\n", i, phdr_cache[i].vaddr, phdr_cache[i].mem_size, phdr_cache[i].file_size);
    }
