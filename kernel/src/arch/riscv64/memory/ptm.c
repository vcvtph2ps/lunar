...
uint32_t g_arch_pte_top_level = 0;
bool g_arch_pte_svpbmt_enabled = false;
...
void ptm_flush_tlb(virt_addr_t vaddr, size_t length) {
    for(; length > 0; length -= ARCH_PAGE_SIZE_4K, vaddr += ARCH_PAGE_SIZE_4K) asm volatile("sfence.vma %0, zero" ::"r"(vaddr) : "memory");
}
...
bool ptm_map(vm_address_space_t* address_space, virt_addr_t vaddr, phys_addr_t paddr, size_t length, vm_protection_t prot, vm_cache_t cache, vm_privilege_t privilege, bool global, bool mmio) {
    ...
    for(size_t i = 0; i < length;) {
        ...
        if(paddr % ARCH_PAGE_SIZE_1G == 0 && vaddr % ARCH_PAGE_SIZE_1G == 0 && length - i >= ARCH_PAGE_SIZE_1G) cursize = ARCH_PAGE_SIZE_1G;
...
void ptm_load_address_space(vm_address_space_t* address_space) {
    uint64_t mode;
    if(g_arch_pte_top_level == 5u)
        mode = ARCH_CSR_SATP_MODE_SV57;
    else if(g_arch_pte_top_level == 4u)
        mode = ARCH_CSR_SATP_MODE_SV48;
    else
        mode = ARCH_CSR_SATP_MODE_SV39;

    ARCH_CSR_WRITE(satp, ARCH_CSR_SATP_MAKE(mode, address_space->ptm.top_level_page_table));
    asm volatile("sfence.vma zero, zero" ::: "memory");
}

static void map_kernel(arch_pte_entry_t* boot_top) {
...
void ptm_init_kernel(uint32_t core_id) {
    ...
    uint64_t boot_satp = ARCH_CSR_READ(satp);
    uint64_t satp_mode = boot_satp & ARCH_CSR_SATP_MODE_MASK;

    assert((satp_mode == ARCH_CSR_SATP_MODE_SV39 || satp_mode == ARCH_CSR_SATP_MODE_SV48 || satp_mode == ARCH_CSR_SATP_MODE_SV57) && "ptm: unexpected satp mode – must be Sv39, Sv48, or Sv57");

    if(satp_mode == ARCH_CSR_SATP_MODE_SV57)
        g_arch_pte_top_level = 5u;
    else if(satp_mode == ARCH_CSR_SATP_MODE_SV48)
        g_arch_pte_top_level = 4u;
    else
        g_arch_pte_top_level = 3u;

    g_arch_pte_svpbmt_enabled = detect_svpbmt();

    LOG_INFO("ptm: paging mode = Sv%u  svpbmt = %s\n", g_arch_pte_top_level == 5u ? 57u : g_arch_pte_top_level == 4u ? 48u : 39u, g_arch_pte_svpbmt_enabled ? "yes" : "no");

    phys_addr_t boot_root_pa = (boot_satp & ARCH_CSR_SATP_PPN_MASK) << 12;
    arch_pte_entry_t* boot_top = (arch_pte_entry_t*) PTM_TO_HHDM(boot_root_pa);
    ...
    ptm_load_address_space(g_vm_global_address_space);
    LOG_OKAY("switched to kernel page tables\n");
    vm_init_kernel();
    ...
    for(size_t i = 0; i < g_init_boot_info->framebuffer_count; i++) {
        ...
        LOG_INFO("mapping framebuffer %zu, 0x%lx → 0x%lx (len 0x%lx)\n", i, paddr, (uintptr_t) new_vaddr, aligned_length);
...
bool ptm_init_user(vm_address_space_t* address_space) {
    ...
    arch_pte_entry_t* user_root = (arch_pte_entry_t*) PTM_TO_HHDM(address_space->ptm.top_level_page_table);
    arch_pte_entry_t* kernel_root = (arch_pte_entry_t*) PTM_TO_HHDM(g_vm_global_address_space->ptm.top_level_page_table);
    for(int i = 256; i < 512; i++) user_root[i] = kernel_root[i];
